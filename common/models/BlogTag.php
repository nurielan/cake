...
class BlogTag extends \yii\db\ActiveRecord
{
    ...
    public function uploadImages($no)
    {
        if ($this->validate()) {
            if ($no == 1) {
                ...
                Image::make($this->imageTemp1->tempName)->save('img/blog_tag/origi/' . $this->imageName1, 95);
                Image::make($this->imageTemp1->tempName)->save('img/blog_tag/thumb/' . $this->imageName1, 75);

                $this->image1 = $this->imageName1;
            } elseif ($no == 2) {
                $this->imageName2 = $this->no . '_' . strtotime(date('Y-m-d h:i:s')) . '_' . $no . '.' . $this->imageTemp2->getExtension();

                Image::make($this->imageTemp2->tempName)->save('img/blog_tag/origi/' . $this->imageName2, 95);
                Image::make($this->imageTemp2->tempName)->save('img/blog_tag/thumb/' . $this->imageName2, 75);

                $this->image2 = $this->imageName2;
            } elseif ($no == 3) {
                $this->imageName3 = $this->no . '_' . strtotime(date('Y-m-d h:i:s')) . '_' . $no . '.' . $this->imageTemp3->getExtension();

                Image::make($this->imageTemp3->tempName)->save('img/blog_tag/origi/' . $this->imageName3, 95);
                Image::make($this->imageTemp3->tempName)->save('img/blog_tag/thumb/' . $this->imageName3, 75);
    ...
    {
        if ($existingImage) {
            unlink('img/blog_tag/origi/' . $existingImage);
            unlink('img/blog_tag/thumb/' . $existingImage);
